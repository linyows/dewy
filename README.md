...
**Docker run options passthrough:** All options after `--` are passed directly to docker run. Forbidden options: `-d`, `-it`, `-i`, `-t`, `-p`. The `dewy.` label prefix is reserved for Dewy's internal container tracking and cannot be used with `-l` / `--label`.
