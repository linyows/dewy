...
# デフォルトの動作（設定不要）
# 自動的に$PWD/.dewy/cacheを使用
```

### メリット

- **永続キャッシュ**: システム再起動やOSクリーンアップ処理に耐える
- **冗長ダウンロードの防止**: 同じバージョンのアーティファクトは一度だけダウンロード
- **デプロイサイクルの削減**: キャッシュに現在のバージョンが含まれている場合はデプロイをスキップ
- **ネットワーク効率**: 長時間実行されるデプロイでの帯域幅使用量を最小化
