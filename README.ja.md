...
ダウンロードしたアーティファクトは、キャッシュへの書き込みと展開の前に、隣に公開されたSHA-256チェックサムファイルと照合されます。アーティファクトごとのファイル（`myapp_linux_amd64.tar.gz.sha256`）と、リリース全体をまとめたファイル（`checksums.txt`、`SHA256SUMS`、`myapp_1.2.3_checksums.txt`）の両方を認識します。動作は`--verify-checksum`で選択します。`auto`（デフォルト）はチェックサムファイルが公開されている場合に検証し、`required`はチェックサムファイルが公開されていない場合にデプロイを失敗させ、`off`は検証しません。コンテナイメージはダイジェストで指定されコンテナランタイムが検証するため、この対象外です。

Cache
--

キャッシュは、現在のバージョンやダウンロード済みアーティファクトをDewyが保持するためのインターフェースです。キャッシュの実装には、ローカルファイル（デフォルト）、AWS S3、Google Cloud Storageが利用可能です。
