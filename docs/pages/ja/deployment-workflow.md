...
検証フェーズは、serverモードで`--health-path`が設定されている場合に実行されます。assetsモードの場合と、このオプションが未設定の場合はスキップされます。

dewyはプロセスの起動または再起動の後に、`http://127.0.0.1:<ポート><--health-path>`をプローブします。ポートは指定された`--port`のうち最小の番号です。フラグの解析時にポート一覧が重複除去され数値順にソートされるためです。ステータス2xxまたは3xxを成功として扱います。成功するか`--health-timeout`（デフォルト30秒）を使い切るまで、2秒間隔で試行を繰り返します。

再起動の場合、プローブの前にworkerの入れ替え完了を待ちます。server-starterは再起動シグナルを受けると、既存のworkerと並べて新しいworkerを起動します。両者は継承したlistening socketを共有し、新しいworkerが起動直後の期間を生き延びた時点で古いworkerを停止します。この期間に送ったプローブは、置き換え対象のリリースが応答する可能性があります。そのためdewyは、server-starterのステータスファイルが「より新しい世代のworkerが1つだけ」を報告するまで最大30秒待ちます。時間内にその状態にならなかった場合はそのままプローブを実行し、結果が前のリリースによるものである可能性を警告としてログに記録します。
...
制限時間内に成功しなかった場合、dewyはデプロイをロールバックします。

1. 失敗したバージョンをキャッシュの`blocked`キーに記録します。この記録がある間、同じバージョンは以降のポーリングでスキップされます。したがって起動しないバージョンのダウンロードとデプロイは、毎回ではなく1回だけ発生します。
2. `current`シンボリックリンクを、デプロイ前に参照していたリリースディレクトリに戻し、`current`キャッシュキーも前のバージョンに戻します。
3. サーバーを再起動し、戻したリリースで動作させます。
4. ヘルスチェックの失敗と、戻したバージョンの両方を重要通知として送信します。
...
記録が保持するのは`<タグ>--<アーティファクト>`形式のキャッシュキーです。同じタグの内容だけを差し替えて再公開してもこのキーは変わらないため、同一タグでの再公開では解除されません。解除される条件は3つです。別のバージョンを公開してそのデプロイが成功したとき、`--health-path`なしで動作させたとき（記録はヘルスチェックが有効な間だけ参照されます）、キャッシュストアの`blocked`エントリを削除したときです。

`--no-rollback`を指定すると、失敗したリリースをそのまま維持します。バージョンの記録と通知は従来どおり行われ、シンボリックリンクの復元と再起動だけがスキップされます。

戻すべき前のリリースが存在しないケースが2つあります。ホスト上での最初のデプロイと、前のバージョンが失敗したバージョンと同一の場合です。この場合、dewyはシンボリックリンクを変更せずに、バージョンの記録と失敗の通知を行います。

ロールバックはヘルスチェックの失敗時だけでなく、プロセスの起動自体に失敗した場合にも実行されます。サーバーが存在する前にデプロイが失敗した場合は再起動の対象がないため、ロールバックは戻したリリースでサーバーを起動します。

バージョンがblockedの間にサーバーが停止していることを検出した場合、`current`シンボリックリンクが指すリリースでサーバーを起動し直します。バージョンをblockedにすることが止めるのはそのバージョンのデプロイであり、ホストが既に持っているリリースの稼働ではありません。
...
| **ロールバック** | 自動ロールバック（ヘルスチェック失敗時）または前のリリースディレクトリ | 自動ロールバック（ヘルスチェック失敗時） |
| **ヘルスチェック** | localhost経由のHTTP（`--health-path`指定時） | HTTP/TCPエンドポイント |
