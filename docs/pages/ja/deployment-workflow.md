...
dewyはプロセスの起動または再起動の後に、`http://127.0.0.1:<最初の--port><--health-path>`をプローブします。ステータス2xxまたは3xxを成功として扱います。成功するか`--health-timeout`（デフォルト30秒）を使い切るまで、2秒間隔で試行を繰り返します。
...
記録はデプロイがヘルスチェックに成功した時点で消去されます。したがって修正後に同じタグで再公開されたバージョンは、再びデプロイされます。
...
ロールバックはヘルスチェックの失敗時だけでなく、プロセスの起動自体に失敗した場合にも実行されます。
