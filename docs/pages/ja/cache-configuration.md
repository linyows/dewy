...
デフォルトではローカルファイルキャッシュが使用されます。複数インスタンス間でキャッシュを共有したい場合は、`--cache`フラグでS3またはGoogle Cloud Storageのbucketを指定します。

```sh
# ローカルファイル（デフォルト）
dewy server --registry ghr://owner/repo -- /opt/app/current/app

# Amazon S3（およびS3互換ストレージ）
...
S3およびGCSの場合、cloud bucketが真実のソースとなります。新しいartifactを最初にダウンロードしたインスタンスがbucketにuploadし、後続のインスタンスは上流registryではなくbucketから取得します。各インスタンスはローカルstagingコピーも保持するため、アーカイブ展開はfile backendと同じ動作になります。

認証は各providerの標準credential chain（AWS環境変数 / shared config / IAM role、Google ADC / `GOOGLE_APPLICATION_CREDENTIALS` / workload identity）を使用します。
