...
dewyはダウンロードしたアーティファクトを、併せて公開されているSHA-256チェックサムファイルと照合します。照合はキャッシュへの書き込みと展開の前に行われます。チェックサムと一致しないファイルはデプロイされず、両方のダイジェストを示すエラーでデプロイが失敗します。

認識するレイアウトは2種類あります。1つはアーティファクト名に対応したファイルです。
...
もう1つは、アーティファクトごとに1行を持つ集約ファイルです。形式は`sha256sum`の出力と同じです。`checksums.txt`と`SHA256SUMS`のほか、`myapp_1.2.3_checksums.txt`のようにリリースツールが生成する名前も認識します。GoReleaserはデフォルトでこの形式を生成します。
...
アーティファクト名に対応したファイルは、集約ファイルより優先されます。前者で認識する拡張子は`.sha256`、`.sha256sum`、`.sha256.txt`、`.sha256sums`です。

検証の挙動は[`--verify-checksum`](/ja/reference#--verify-checksum)で制御します。デフォルトの`auto`は、チェックサムファイルが公開されていれば検証し、なければ検証せずにデプロイします。`required`は、チェックサムファイルが公開されていない場合もデプロイを失敗させます。`off`は検証を行いません。

検証の対象は新規ダウンロードのみです。すでにキャッシュにあるアーティファクトは、最初にダウンロードした時点で検証済みです。これは同じキャッシュバックエンドを共有する別のインスタンスがダウンロードした場合も同様です。

コンテナイメージはこのオプションの対象外です。イメージはダイジェストで指定され、pull時にコンテナランタイムが検証します。

セキュリティを重視する環境では、GPG署名を併せて提供することも可能です。dewyはGPG署名を検証しません。検証が必要な場合はデプロイフックを使用してください。
