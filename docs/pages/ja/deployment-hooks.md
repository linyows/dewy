...
フックが失敗した場合、アーティファクトの展開は行われず、`current` シンボリックリンクは稼働中のリリースを指したままになります。Dewyはその実行を失敗として記録し、エラー通知を送信したうえで、次のポーリングで同じバージョンを再試行します。一時的な原因でデプロイが止まった場合は、その原因が解消すればデプロイされます。連続して失敗するとポーリングのバックオフが働き、再試行の間隔が広がります。
...
- アプリケーションの起動・再起動より前（server コマンドの場合）
- ローリングデプロイ完了後（container コマンドの場合）
...
### ロールバック時の動作 {% #rollback-behavior %}

ロールバックによって直前のリリースが復元されるとき、フックは実行されません。

`server` コマンドで `--health-path` を指定している場合、Dewyはアプリケーションを再起動した後に新しいリリースのヘルスチェックを行います。このヘルスチェックが失敗すると、Dewyはそのバージョンをブロック済みとして記録し、直前のリリースディレクトリを復元し、`current` シンボリックリンクを張り替えて、サーバーを起動し直します。この一連の処理では、Before Deploy HookもAfter Deploy Hookも実行されません。

**server コマンドでのデプロイの処理順:**

1. Before Deploy Hook
2. アーティファクトの展開と `current` シンボリックリンクの切り替え
3. After Deploy Hook
4. アプリケーションの起動または再起動
5. ヘルスチェック（`--health-path` 指定時のみ）
6. 手順5が失敗した場合のロールバック

After Deploy Hookは手順3、つまりアプリケーションの再起動よりも前、ヘルスチェックよりも前に実行されます。そのため、手順6で直前のリリースが復元される時点では、新しいリリースに対するAfter Deploy Hookはすでに実行済みです。ロールバック後は次の状態になります。

- After Deploy Hookは、すでに稼働していないリリースに対して実行された状態になる
- 現在稼働している復元済みリリースに対しては、どのフックも実行されていない
- リリースパスから状態を組み立てるフック（独自のシンボリックリンク、キャッシュのウォームアップ、生成した設定ファイルなど）は、失敗したリリースを指したまま残る

通知は復元後のリリースパスで更新されるため、通知内容は実際に稼働しているリリースを示します。

{% callout type="warning" title="フックとロールバック" %}
After Deploy Hookは、生成する状態が特定のリリースディレクトリに依存しないようにするか、ロールバックの通知を受けた後に手動で修正できる形にしておいてください。
ロールバック専用のフックを設けるかどうかは issue #493 で検討中です。
{% /callout %}

`container` コマンドではこの状態は発生しません。ローリングデプロイの失敗はデプロイ処理の内部でロールバックされ、それはAfter Deploy Hookの実行より前であるため、切り戻されたリリースに対してフックが実行されることはありません。
