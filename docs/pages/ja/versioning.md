...
リリースチャンネルとは、プリリリース識別子の先頭の要素です。`v1.2.3-canary.1`と`v1.2.3-canary.2`はどちらも`canary`チャンネルに属します。`v1.2.3-rc.1`は`rc`チャンネルです。`v1.2.3`のようにプリリリース識別子を持たないバージョンは`stable`チャンネルに属します。

`--channel`はインスタンスを1つのチャンネルに限定します。これにより、全台に配る前に一部のホストへ先行してバージョンを展開できます。

```bash
# カナリアホスト - v*-canary.* のみをデプロイする
dewy server --registry ghr://owner/repo --channel canary -- /opt/myapp/current/myapp

# その他のホスト - 正式リリースのみをデプロイする
dewy server --registry ghr://owner/repo --channel stable -- /opt/myapp/current/myapp
```

`v1.4.0-canary.1`を公開すると、カナリアホストだけがそれをデプロイします。`stable`のホストは、`v1.4.0`が公開されるまで`v1.3.0`のままです。昇格にホスト側の変更は不要です。正式なタグを公開することが昇格の操作にあたります。

`stable`以外のチャンネルは定義上プリリリースバージョンを選択するため、`--channel canary`にレジストリURLの`pre-release=true`は不要です。`--channel stable`は、レジストリURLが`pre-release=true`を指定していてもプリリリースバージョンを除外します。`--channel`を指定しない場合、バージョン選択の挙動は従来どおりです。レジストリの`pre-release`オプションが、プリリリースバージョンを候補に含めるかどうかを決めます。

チャンネル名は大文字と小文字を区別せずに比較されます。

{% callout type="note" title="チャンネルとスロット" %}
チャンネルとデプロイメントスロットは互いに独立しており、併用できます。チャンネルはプリリリース識別子であり、そのバージョンがどこまで展開されたかを表します。スロットはビルドメタデータであり、並行する2つの環境のどちらに向けたビルドかを表します。両方のオプションを指定して起動したインスタンスは、両方の条件を満たすバージョンのみをデプロイします。`--channel canary --slot blue`の場合は`v1.4.0-canary.1+blue`です。
{% /callout %}
