...
server 変種の1つは追加で `--telemetry` を付けて起動し、`/metrics` エンドポイントが server モードの系列を公開していることを検証します：`command="server"` ラベル付きの `dewy_deployments_total`、`reason="deploy"` ラベル付きの `dewy_server_restarts_total`、および `dewy_server_up`。
...
- TCPプロキシが正しく機能している
- マルチポートルーティングが動作している
- テレメトリの `/metrics` エンドポイントが `dewy_container_*` ライフサイクル系列を公開している
